...
namespace PegasusLib.Content {
	public class DyedLight {
		...
		public void GetColorWithRateLimit(ref Vector3 color, ref ulong timer, Func<int?> armorShaderID, Player player, Entity entity) {
			if (PegasusConfig.Instance.ProceduralLightSourceDyeRate != 0) {
				unchecked {
					if (PegasusLib.gameFrameCount - (ulong)PegasusConfig.Instance.ProceduralLightSourceDyeRate < timer) return;
				}
			}
			timer = PegasusLib.gameFrameCount;
			color = GetColor(armorShaderID, player, entity);
		}
		public Vector3 GetColor(Func<int?> armorShaderID, Player player, Entity entity) {
			if (Main.dedServ) return BaseColor;
			if (armorShaderID() is not int dye) return BaseColor;
			ArmorShaderData shader = GameShaders.Armor.GetSecondaryShader(dye, player);
			if (shader is null) return BaseColor;
			if (PegasusConfig.Instance.ProceduralLightSourceDyeRate == 0 || dyeFunctionIsBetter.Contains(shader)) {
				if (dyeFunctions.TryGetValue(shader, out DyeFunction dyeFunction)) return dyeFunction(BaseColor, entity, this);
				return BaseColor;
			}
		...
		internal static void Initialize() {
			...
			for (int i = 0; i < shaderData.Count; i++) {
				...
				if (FindDistinctPass(shader.Shader.Techniques.First().Passes) == distinctPassIndex) {
					if (ShaderSets.Armor.BasicColorDye[i]) {
						...
						case "ArmorStardust": {
							...
							dyeFunctions[shader] = (color, entity, _) => {
								...
								float noise = PerlinNoise.Get(
									(int)Main.timeForVisualEffects % PerlinNoise.Width,
									(entity.whoAmI * 7 * 5) % PerlinNoise.Height
								);
								sparkleColor.Y *= saturation_mult - float.Pow(noise, 2) * saturation_mult * 0.85f;
		...
		public static void Cache<TItem>() where TItem : ModItem {
		...
		public static void PreferDyeFunction<TItem>(DyeFunction function) where TItem : ModItem {
			ArmorShaderData shader = GameShaders.Armor.GetShaderFromItemId(ModContent.ItemType<TItem>());
