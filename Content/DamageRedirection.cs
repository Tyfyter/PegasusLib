...
public abstract class DamageRedirection : IModType, ILoadable {
	...
	public abstract void Apply(ref double damage, in Player.HurtInfo info);
	public virtual void HookOnDamageEffects() { }
	public virtual IEnumerable<DamageRedirection> SortAfter() => [
		ModContent.GetInstance<EarlyHealthReplacement>()
	];
	public virtual IEnumerable<DamageRedirection> SortBefore() => [
		ModContent.GetInstance<FinalHealthReplacement>()
	];
	...
	internal static void Sort() {
		...
		orderedDamageRedirections.RemoveAll(dr => dr is DamageRedirectionDivider);
...
public static class DamageRedirectionLoader {
	...
	public static double Apply(Player player, double damage, in Player.HurtInfo info) => player.GetModPlayer<DamageRedirectionPlayer>().Apply(damage, in info);
	...
	static bool CalculateRedirection(Player player, double damage, in Player.HurtInfo info) {
		currentDamage = Apply(player, damage, in info);
...
class DamageRedirectionPlayer : ModPlayer {
	...
	public double Apply(double damage, in Player.HurtInfo info) {
		for (int i = 0; i < redirections.Count; i++) redirections[i].Apply(ref damage, in info);
		return damage;
	}
}
...
public abstract class DamageRedirectionToMana : PercentageDamageRedirection {
	...
	public override void Apply(ref double damage, in Player.HurtInfo info) {
...
/// <summary>
/// Anything which extends this class will be removed from the final list
/// </summary>
public abstract class DamageRedirectionDivider : DamageRedirection {
	public sealed override void Apply(ref double damage, in Player.HurtInfo info) { }
	public sealed override void ResetEffects() { }
	public override IEnumerable<DamageRedirection> SortAfter() => [];
	public override IEnumerable<DamageRedirection> SortBefore() => [];
}
public class EarlyHealthReplacement : DamageRedirectionDivider { }
public class FinalHealthReplacement : DamageRedirectionDivider { }
public abstract class OnDamageEffectModifier : ILoadable {
	...
	public delegate void DamageModifier(ref double damage, in Player.HurtInfo info);
	public int ModifyUsedDamage(int damage, Player player, in Player.HurtInfo info) {
